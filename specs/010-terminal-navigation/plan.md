...
**Bugfix**: 2026-09-03 — BUG-009 Updated from bugfix patch
...
По BUG-009 command-rejection acknowledgement имеет приоритет над root terminal-return interception даже при непустом маршруте. Rejected renderer очищает наследованные return label/class/semantics, coordinator сначала очищает exact `CommandExecutionPresentation`, и только отдельный `Back` из восстановленного root menu может создать return pending. После отдельного возврата и повторной активации terminal checkpoint сохраняет solved hack, но не публикует подтверждённый или lifecycle-cancelled stale rejection; новые protocol fields не требуются.
