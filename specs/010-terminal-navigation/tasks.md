...
**Bugfix**: 2026-08-28 — BUG-008 Updated from bugfix patch; completed state-changing reject/close MUST publish «Ошибка доступа» until controller acknowledgement.
**Bugfix**: 2026-09-03 — BUG-009 Updated from bugfix patch; rejected-command acknowledgement MUST precede root terminal return and transient rejection MUST NOT survive return/re-entry.
...
- [x] **T019** [P] [US2] ⚠️ Reopened — Добавить tests для first-entry hack, solved/unfinished/failed reactivation, explicit reset/discard, root placement и single-decision source checkpoint preservation; по BUG-009 доказать, что return/re-entry сохраняет solved hack, но не восстанавливает acknowledged/lifecycle-cancelled `CommandExecutionPresentation` `(reopened — BUG-009)` · `internal/live/service_test.go`, `internal/control/service_test.go`
...
- BUG-008 corrective DAG overrides the completed-state-change direct-menu rejection baseline: parallel failing regressions `T053/T054` plus reopened `T041/T047/T048` → reopened `T042` coordinator correction → reopened `T043` projection/reconnect audit → `T055` focused cross-surface and notification verification → reopened `T044` mode matrix → reopened `T030` full gate → reopened `T031` native smoke → already-open `T036` packaged-runtime gate.
- BUG-009 corrective DAG overrides the isolated ordinary-rejection/return coverage: parallel failing composed regressions `T056/T057` plus reopened `T047/T048` → reopened `T042` acknowledgement-precedence correction → reopened `T019/T043` reactivation/player correction → `T058` focused composed verification → reopened `T049/T044` cross-mode gates → reopened `T030` full gate → reopened `T031` native `sessions/demo.json` smoke → reopened `T036` packaged-runtime gate.
...
- BUG-008 failing Go/stream and browser regressions `T053/T054` are independent; correction and verification then follow the BUG-008 DAG, while completed approve/frozen-result behavior and terminal-return rejection stay unchanged guards.
- BUG-009 failing coordinator/stream regression `T056` and real-demo browser regression `T057` are independent; implementation remains ordered through `T042 → T019/T043 → T058`, while `T049/T044/T030/T031/T036` consume the composed SC-018 evidence.
...
- [x] **T049** [US1] [US4] ⚠️ Reopened — После T042/T043 прогнать focused ordinary reject/close Go, stream и browser матрицу, доказать notification-action reject через тот же authoritative App decision path без отдельной player semantics, проверить lifecycle clearing; по BUG-009 включить non-empty route acknowledgement precedence и return/re-entry cleanup, затем передать SC-015/SC-018 в reopened T044 → T030 → T031 → T036 `(reopened — BUG-009)` · `approval_notifications_test.go`, `app_test.go`, `internal/control/service_test.go`, `internal/player/public_stream_test.go`, `tests/browser/state-changing-command-approval.spec.mjs`, `tests/browser/terminal-navigation.spec.mjs`

**Checkpoint**: Ordinary explicit reject, close-as-reject и notification reject показывают всем «Ошибка доступа» до controller acknowledgement, затем возвращают неизменённое source menu без command, persistence, terminal или route effect; ~~completed-state-change,~~ ~~terminal-transition~~ и return rejection semantics не изменены. BUG-007 supersedes terminal-transition rejection semantics; BUG-008 supersedes completed-state-change rejection semantics.
...
**Checkpoint**: Forward terminal-transition explicit reject, close-as-reject и notification reject показывают всем «Ошибка доступа» до controller acknowledgement, затем возвращают неизменённое source menu без terminal activation, route push, checkpoint or persistence effect; ~~completed-state-change и~~ terminal-return rejection semantics не изменены. BUG-008 supersedes completed-state-change rejection semantics.

## Phase 17: BUG-008 — completed state-changing rejection access-error presentation

**Purpose**: Сохранить repeated completed state-changing command effect-free при reject/close, но показать всем игрокам server-authoritative полноэкранный экран «Ошибка доступа» до controller acknowledgement вместо немедленного возврата в source menu, не меняя completed approve/frozen-result semantics.

### Wave 1 — independent failing regressions

- [x] **T053** [P] [US1] [US4] Добавить падающий coordinator/player regression для repeated completed state-changing explicit reject и close-as-reject: exact `PendingCommandExecution` очищается, active runtime публикует `CommandExecutionPhaseRejected` для exact completed command, controller + two observers + reconnect сходятся на projection, frozen result и source menu скрыты, source nav и command state остаются неизменными, repeat execution, durable write, terminal и route effects равны нулю; отдельно доказать controller `Back`/`Enter` acknowledgement и unchanged completed-approve frozen-result guard · `internal/control/service_test.go`, `internal/player/adapter_test.go`, `internal/player/public_stream_test.go`
- [x] **T054** [P] [US1] [US4] Добавить падающий Playwright regression repeated completed state-changing explicit reject/close: у controller + two observers + reconnect source menu и frozen result скрыты, body точно равен «Ошибка доступа» до controller `Back` и отдельного `Enter` journey, observers read-only, затем все синхронно возвращаются в byte-for-byte unchanged source menu без repeat execution/write/command-state effect; сохранить completed approve result journey · `tests/browser/terminal-navigation.spec.mjs`, `tests/browser/fixture-server/main.go`

**⟶ T053/T054 and reopened T041/T047/T048 must fail for direct-menu completed rejection before executing reopened T042 → T043.**

### Wave 2 — focused cross-surface verification

- [x] **T055** [US1] [US4] После T042/T043 прогнать focused completed state-changing reject/close Go, stream и browser матрицу, доказать notification-action reject через тот же authoritative App command-execution decision path без отдельной player semantics, проверить lifecycle clearing, отсутствие protobuf/generated delta и неизменный completed-approve frozen-result baseline, затем передать SC-017 в reopened T044 → T030 → T031 → T036 · `approval_notifications_test.go`, `app_test.go`, `internal/control/service_test.go`, `internal/player/public_stream_test.go`, `tests/browser/terminal-navigation.spec.mjs`

**Checkpoint**: Repeated completed state-changing explicit reject, close-as-reject и notification reject показывают всем «Ошибка доступа» до controller acknowledgement, затем возвращают неизменённое source menu без frozen result leakage, repeat execution, durable write, command-state, terminal или route effect; completed approve и terminal-return rejection semantics не изменены.

## Phase 18: BUG-009 — routed-root rejection acknowledgement and clean re-entry

**Purpose**: На terminal root с непустым route подтверждать `REJECTED` до распознавания межтерминального возврата, показывать generic acknowledgement control и сохранять solved hack checkpoint без stale rejection после отдельного return/re-entry.

### Wave 1 — independent failing composed regressions

- [x] **T056** [P] [US1] [US2] [US4] Добавить падающий coordinator/live/player regression для `A → protected B`, solved hack, ordinary reject в root B с непустым route и отдельных `Back`/`Enter`: первое действие очищает exact `CommandExecutionPhaseRejected`, не создаёт `PendingTerminalNavigation`, не меняет active terminal/route/checkpoint и синхронно публикует root menu controller/two observers/reconnect; затем отдельный return и forward re-entry сохраняют solved hack при `CommandExecution == nil` · `internal/control/service_test.go`, `internal/live/service_test.go`, `internal/player/public_stream_test.go` per FR-012, FR-015, FR-023, FR-037/BUG-009, SC-003, SC-018
- [x] **T057** [P] [US1] [US2] [US3] Добавить падающий Playwright regression на `sessions/demo.json`: одобрить `t_demo1 → t_demo2`, завершить hack, отклонить `n_cmd_security_summary`, доказать exact «Ошибка доступа» с generic back control без `terminal-return` class/label, проверить `Back` и отдельный `Enter`, затем отдельный approved return в `t_demo1` и approved re-entry в `t_demo2` с root menu без нового hack/stale rejection у controller/two observers/reconnect · `tests/browser/terminal-navigation.spec.mjs`, `tests/browser/fixture-server/main.go`, `sessions/demo.json` per User Stories 1–3, FR-012, FR-015, FR-037/BUG-009, SC-018

**⟶ T056/T057 and reopened T047/T048 must fail on acknowledgement precedence, inherited return control, or stale re-entry before executing reopened T042 → T019/T043.**

### Wave 2 — focused composed verification

- [x] **T058** [US1] [US2] [US4] После reopened T042/T019/T043 прогнать focused Go/stream/browser `Back|Enter` matrix, проверить explicit/close/notification ordinary rejection at routed target root, exact revision ordering, отсутствие return pending/route mutation на acknowledgement, отдельный approved return и re-entry с retained solved hack and nil command presentation; затем передать SC-018 в reopened T049 → T044 → T030 → T031 → T036 · `approval_notifications_test.go`, `app_test.go`, `internal/control/service_test.go`, `internal/live/service_test.go`, `internal/player/public_stream_test.go`, `tests/browser/terminal-navigation.spec.mjs`

**Checkpoint**: Rejection acknowledgement никогда не маскируется root return control, отдельный terminal return начинается только из восстановленного root menu, а повторная активация сохраняет hack checkpoint без stale `REJECTED` presentation.
