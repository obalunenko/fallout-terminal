...
- **FR-045**: The shared strict compiler policy, application-owned declarations, both Vue shells, and the typed desktop API adapter MUST be established before Overseer leaf components and composables are migrated; no shared authored application declaration module or type-only cross-boundary application contract may be introduced.
