...
- Q: In what order must the two applications be migrated? → A: Infrastructure, TypeScript protobuf generation, shared strict compiler policy plus application-owned declarations and typed desktop API, Overseer components, complete Overseer cutover, Player foundations, Player presentation adapters, complete Player cutover, then final cleanup and verification.
- Q: What Player work is permitted before the complete Overseer wave-e exit? → A: Wave c may create only capability-neutral strict-compiler participation, Player-owned declarations and ports, an empty Vue shell, a mount function, an isolated candidate/test document, and dependency/boundary policy fixtures. Those foundations must contain no Player business behavior, must not replace or mutate production Player DOM, must not consume Wails or another privileged capability, and must not become production-selected. Player feature migration begins only after the complete wave-e exit gate passes.
...
- Q: What makes a generated task executable and traceable? → A: Each implementation task names complete repository-relative modifiable paths, separate read-only inputs, prerequisites whose producers already exist, a locally executable completion check, and explicit valid FR/SC/checklist task mappings; test-first tasks additionally record an accepted product RED signature, reject infrastructure/configuration failures, and name the later GREEN task.
- Q: How are Node and Go workflow rules enforced? → A: `node:check` accepts exactly `26.8.1` and self-tests that this version passes while an older and a newer version fail. Before every commit containing Go-source changes, the owning task runs `go fix ./...`, reviews and retains only intentional modernization edits, formats, and executes its task-specific Go gates; the final Go task audits rather than substitutes for earlier compliance.
- Q: Do active Spec Kit template updates cross the maintenance boundary? → A: No. Feature 033 may update only the already-authorized `.specify/templates/plan-template.md`, `.specify/templates/spec-template.md`, and `.specify/templates/tasks-template.md`, each as a separate exact-file maintenance task; the constitution and completed historical specifications/evidence remain read-only.
...
- **FR-031**: Every intermediate migration revision MUST be buildable and covered by its applicable automated tests, and every implementation task MUST have a locally executable completion check when that task runs rather than relying only on a later dependent integration task.
...
- **FR-033**: Every migration wave MUST record its Vue mount boundaries and remaining legacy ownership, and intermediate JavaScript and TypeScript coexistence MUST remain bounded by an exact tracked migration inventory naming each owning file, selector or entrypoint, creation task, owner, permitted scope, expiry wave, unconditional removal task, and absence verification without weakening the final production compiler configuration.
...
- **FR-035**: Active developer and release documentation, dependency workflows, build scripts, generation scripts, validation scripts, continuous-integration checks, and the three already-authorized active Spec Kit templates MUST describe and enforce the migrated applications, exact Node.js `26.8.1`, locally executable task verification, separate exact modifiable/read-only repository-relative file inventories, explicit valid FR/SC/checklist task mappings, test-first RED/GREEN semantics, task-local Go pre-commit compliance, and safe optional parallelism. The three template files MUST be handled as separate maintenance tasks and MUST NOT authorize changes to the constitution or completed historical artifacts.
...
- **FR-042**: TypeScript, `@vitejs/plugin-vue`, and `vue-tsc` MUST be exact direct development dependencies in their plan-determined workspace ownership, Node.js MUST remain exactly `26.8.1` across `.nvmrc`, Task preflight, CI, manifests, lockfile expectations, quickstart, and active documentation, deterministic self-tests MUST prove `26.8.1` passes while an older and a newer version fail, and the migration MUST NOT upgrade unrelated dependencies.
- **FR-043**: Migration work MUST begin with Vue and TypeScript infrastructure plus bounded temporary legacy checking before application component cutover; bootstrap validation MUST use commands and checkers that already exist at the point each task executes, after which the canonical Task targets and their aggregate contract tests are introduced.
...
- **SC-011**: Every reviewed intermediate revision has one recorded owner per rendered subtree, a successful applicable production build, a passing locally executable task-level check plus any broader wave evidence, and an exact temporary/legacy inventory; final acceptance has zero entries remaining in that inventory.
