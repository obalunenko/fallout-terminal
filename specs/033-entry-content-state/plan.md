...
Extend session version 1 so an entry may contain ordered, stable text blocks and a state-changing command may own one optional block transition. Freeze that transition inside the command's existing durable execution snapshot, compose effective entry text on the server, and publish it through the unchanged player entry-description contract. In the Overseer editor, derive human-readable cross-references from that one canonical relationship: commands show the entry name, block position, and authored-text preview, while targeted blocks show the targeting command name. Move command-state resets through the coordinator's existing persistence seam so execution and reset publish only after one atomic session write and cannot expose command/block partial state.
...
├── overseer.js                                      # block editor, bidirectional target labels, deletion guards
└── overseer.css                                     # compact block, target, and owner-reference controls
...
**Structure Decision**: Keep authored and durable state in the existing domain/session ownership boundary, keep coordination and effective projection in control/live, expose no new player or desktop methods, and limit browser changes to Overseer authoring plus verification of the existing player rendering path.

## Constitution Check

| Principle | Before research | After design | Assessment |
|---|---|---|---|
| I. Govern the Accepted Desktop Runtime | PASS | PASS | Domain, session, live, and control remain Wails-independent; the Overseer continues through the narrow registered desktop service. |
| II. Make Protobuf the Application Contract Source of Truth | PASS | PASS | New persisted structures begin in `session.proto`; generated code stays at boundaries and is never edited manually. |
...
| IV. Separate Public and Private Capabilities | PASS | PASS | Players receive only flattened effective entry text. Authored targets, alternate content, session state, approval, and reset remain private. |
| V. Evolve Schemas Safely and Reproducibly | PASS | PASS | Existing field numbers remain stable, all additions use new field numbers and explicit message presence, and pinned generation/drift/breaking checks are required. |
...
| VII. Complete Cutovers and Remove Superseded Protocols | PASS | PASS | No parallel state map, public protocol, or desktop method is introduced; legacy description is a compatibility input, not a second active representation after conversion. |
...
[research.md](./research.md) records the decisions for command-owned block snapshots, terminal-scoped block identity, legacy conversion, server-side composition, coordinator-owned reset publication, completed-target locking, and derived bidirectional authoring labels.

## Phase 1: Contracts and Data Model

- [data-model.md](./data-model.md) defines authored blocks, optional command targets, frozen snapshots, validation, effective composition, and state transitions.
...
| Overseer authoring | Focused Playwright authoring journey | Add/edit/reorder/reopen, terminal-only targeting, command-side entry/position/preview labels, entry-side command-name ownership labels, identical/empty preview handling, conflict feedback, completed-target lock, deletion guard, and reset confirmation. |
