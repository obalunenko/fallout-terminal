...
**Rationale**: The feature is explicitly authorized, but the current constitution still declares macOS 13+ arm64 as the only supported deployment profile and explicitly prohibits Taskfiles. Updating both governing decisions before implementation keeps code, CI, and project policy aligned. Keeping the existing macOS path distinct minimizes regression risk to signed app, DMG, notarization, and reproducibility checks.

**Alternatives considered**: Implementing under standing deployment and Taskfile violations was rejected because both are governance decisions; folding macOS into the new unsigned portable archives was rejected because it would change a mature signed-release contract outside this feature; weakening existing macOS checks was rejected because FR-015 requires preservation rather than replacement.
