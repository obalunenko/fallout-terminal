...
| SC-013 — tagged five-target GitHub Release and GHCR publication | NOT RUN | T060 now implements the one-SHA unsigned Darwin DMG with exact SHA-256 verification plus four-target portable join, exact 11-file inventory, GoReleaser-only GitHub Release, ORAS-owned versioned GHCR publication, prerelease handling, and a `validate_release=true` non-publishing rehearsal without signing/notarization secrets. T055 must replace this entry only with clean-revision joined-workflow evidence; no tag or production publication was executed. |
