...
Каждая упорядоченная группа представляет один независимый физический терминал или терминальную установку. Элементы внутри группы — это уровни допуска либо локальное и удалённое представления того же терминала; их можно переставлять и переносить между группами. Отдельные машины, даже находящиеся в одном здании, должны состоять в отдельных группах. Переходы вперёд и назад меняют контекст доступа внутри одной группы, сохраняют заданный порядок и требуют подтверждения смотрителя.

## Объект, устройства и диагностика

Раздел **ОБЪЕКТЫ** в Overseer описывает общее состояние локации для всех терминалов сессии. В нём можно создать дверь, турель, энергосеть, реактор, вентиляцию, тревогу, робота, лифт, сетевой сегмент или устройство другого типа. Для устройства задаются стабильный ID, начальное состояние, конечный набор состояний и разрешённые переходы.

1. Создайте устройства и переходы, затем назначьте командам один или несколько переходов. Такая команда изменит весь выбранный набор устройств только после одобрения и успешного сохранения.
2. Добавьте привязки состояния к названию пункта меню, тексту записи, видимости или доступности команды. Один и тот же результат сразу применяется во всех терминалах и группах.
3. Создайте диагностические условия, выберите блокируемые возможности, диагностические записи и явные пути восстановления. Визуальная нестабильность и повреждённые варианты записей меняют только показ уже созданного содержимого.
4. Нажмите **Сохранить объекты**. Overseer проверит устройства, состояния, переходы, условия, привязки и пути восстановления как один граф; при ошибке сохранённая версия останется без изменений.

Кнопка **Предпросмотр** показывает выбранное состояние устройства или диагностическое условие для одного терминала без сохранения и без изменения экрана игроков. Перед удалением или сменой стабильного ID используйте **Зависимости**: Overseer перечислит все ссылки и потребует явно удалить, исправить или переназначить каждую из них.

Для активного диагностического условия выберите предусмотренный автором путь восстановления. Команда игрока проходит обычное подтверждение смотрителя; совместимая программа голодиска выполняет только назначенное действие; приватное восстановление в Overseer требует отдельного подтверждения. **Сбросить устройство** возвращает выбранное устройство и связанные с ним условия к начальным значениям. **Сбросить весь объект** возвращает начальные значения всего объекта. Оба сброса применяются целиком и сохраняются до публикации нового состояния игрокам.

### Журнал действий игроков и объекта

Кнопка **Открыть логи** доступна на стартовом и основном экранах Overseer. Она открывает каталог журналов и показывает путь к текущему файлу; если текущий файл недоступен, интерфейс сообщает об этом и оставляет путь к каталогу для ручного доступа.

Записи позволяют сопоставить запрос, решение, результат, безопасные ID устройств и ревизии по общему идентификатору. В журнал не попадают имена персонажей, авторские названия и тексты, пароли, секреты и подробности внутренних ошибок. Журнал служит только для диагностики: состояние объекта всегда восстанавливается из session JSON.
...
- [История миграции Wails v2 → v3](docs/wails-v3-migration-rollback.md) и [исторический quickstart/acceptance record](specs/006-wails-v3-migration/quickstart.md) — сохранённые свидетельства приёмки beta.8, а не действующие инструкции отката или настройки;
