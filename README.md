...
Сверху показан Overseer, снизу — Player. Игрок выбирает персонажа, читает запись и возвращается в меню; смотритель отклоняет обычную команду, подтверждает изменение состояния и разрешает переход в защищённый терминал. Во время взлома игрок удаляет ложное слово скобочной комбинацией и делает попытку, после чего смотритель подтверждает успех. Возврат в первый терминал одобряется, а повторный переход — отклоняется.
...
Если поле домена пустое, приложение не запрашивает конкретный адрес: ngrok автоматически назначает
хост для публичной ссылки. Подробнее — в документации ngrok об
[автоматически назначенных доменах](https://ngrok.com/docs/gateway/domains/#auto-assigned-domains).
Фиксированный хост используется только после того, как вы заполните поле домена и сохраните
настройки; домен должен быть доступен вашему аккаунту ngrok. Подробнее — в документации ngrok о
[фиксированных доменах](https://ngrok.com/docs/gateway/domains/#domains).
